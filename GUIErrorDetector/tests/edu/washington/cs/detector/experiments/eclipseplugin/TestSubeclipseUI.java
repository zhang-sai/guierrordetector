...
//		entries.addAll(jobMethods);
	...
	protected Collection<CGNode> getStartNodes(Iterable<CGNode> allNodes,
		...
		Collection<Entrypoint> actionEntrypoints = EclipsePluginUtils.getAllPublicProtectedActionEntrypoints(cha, super.getPackages(), false); //non-UI
		Collection<Entrypoint> listenerEventHandler = EclipsePluginUtils.getAllPublicProtectedEventHandlerMethods(cha, super.getPackages(), false);
		Collection<Entrypoint> jobChangeEntries = EclipsePluginUtils.getAllJobChangeListenerMethods(cha, super.getPackages(), false); //non-UI
		...
		System.out.println("Resource listener number");
		Collection<CGNode> resourceChangeMethods = EclipsePluginUtils.getAllResourceChangeMethods(graph, cha, super.getPackages());
		Utils.dumpCollection(resourceChangeMethods, System.out);
		
		System.out.println("Action run number");
		Collection<CGNode> actionMethods = EclipsePluginUtils.getAllActionRunMethods(graph, cha, super.getPackages());
		Utils.dumpCollection(actionMethods, System.out);
		//System.exit(0);
